...
{{--                        <p class="text-base/7 font-semibold text-indigo-600">Deploy faster</p>--}}
                        <h1 class="mt-2 text-4xl font-semibold tracking-tight text-pretty text-gray-900 sm:text-5xl">{{$event->name}}</h1>
                        <p class="mt-6 text-xl/8 text-gray-700">{!! $event->short_description !!}</p>
                    </div>
                </div>
            </div>
            <div class="-mt-12 -ml-12 p-12 lg:sticky lg:top-4 lg:col-start-2 lg:row-span-2 lg:row-start-1 lg:overflow-hidden">
                <img class="w-[48rem] max-w-none rounded-xl bg-gray-900 ring-1 shadow-xl ring-gray-400/10 sm:w-[57rem]" src="{{$event->attachments()->first()->url()}}" alt="">
            </div>
